...
public class ReviewController {
    ...
    /**
     * 로그인된 사용자의 리뷰 리스트 불러오기
     * @param loginIdMap
     * @return
     */
    @PostMapping("/reviews/myList")
    public ResponseEntity<CustomResponseBody<GetMyReviewListDTO>> getMyReviewList(@RequestBody Map<String, String> loginIdMap) {
        CustomResponseBody<GetMyReviewListDTO> responseBody = new CustomResponseBody<>("내 리뷰 리스트 불러오기 성공");
        try {
            List<GetMyReviewListDTO> myReviews = reviewService.getMyReviewList(loginIdMap.get("loginId"));
            responseBody.setList(myReviews);
